...
- `0xf0` - состояние игры. `0` - настройка. `1` - симуляция
- `0xf1` - количество клеток для оживления (битовый массив, где `i`-й бит говорит о `i+1` количестве клеток для выполнения условия)
- `0xf2` - количество клеток для продолжения жизни (битовый массив, где `i`-й бит говорит о `i+1` количестве клеток для выполнения условия)
- `0xf3` - координата Y
- `0xf4` - координата X (*скорее всего не будет нужен*)
- `0xf5`-`0xf8` - 4 регистра для выбранной строки (при запросе на чтение будет загружаться из буфера, при запросе на запись будет отправлять значение во временное хранилище)
- `0xf9` - бит подтверждения полноты введённой строки. Когда сюда отправляется запрос на запись, временное хранилище строки отправит строку в видеобуффер